...
router.get("/:userId", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Bildirimler alınamadı. Lütfen tekrar deneyin." });
  }
});
...
router.put('/:notificationId', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Bir hata oluştu. Bildirim okundu olarak işaretlenemedi." });
  }
});
