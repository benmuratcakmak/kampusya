...
export const Search = () => {
  ...
  const debounceSearch = useCallback((value) => {
    if (timeoutRef.current) clearTimeout(timeoutRef.current); // Önceki timeout'u temizliyoruz
    ...
    timeoutRef.current = setTimeout(async () => {
      if (value.length >= 3) {
        ...
        try {
          ...
          setUserFilter([]); // Hata durumunda arama sonuçlarını sıfırlıyoruz
        } finally {
          setLoading(false);
        }
      } else {
        setUserFilter([]); // Arama uzunluğu 3'ten azsa sonuçları temizliyoruz
      }
    }, 500);
  }, []);
